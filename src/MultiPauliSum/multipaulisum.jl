...
`propagate!()` keeps a worker task on every thread for the whole circuit and hands it the same zones on every gate (see `withworkers()`), so a zone stays on the thread, and on the memory, where it started.
...
MultiPauliSum(psum::AbstractPauliSum, n_zones::Integer=defaultnzones()) =
    add!(_emptyzones(psum, nqubits(psum), n_zones), psum)
...
MultiPauliSum(msum::MultiPauliSum, n_zones::Integer=defaultnzones()) =
    add!(_emptyzones(first(zones(msum)), nqubits(msum), n_zones), msum)
...
convertcoefftype(::Type{CT}, msum::MultiPauliSum) where {CT} =
    MultiPauliSum(nqubits(msum), map(zone -> convertcoefftype(CT, zone), zones(msum)), zonemap(msum))
