...
# Convert one classical argument from the OpenQASM AST into a Julia expression string.
_getstr(vec::Tuple) = join(_getstr(e) for e in vec)
_getstr(tok::OpenQASM.RBNF.Token) = tok.str
_getstr(tok::OpenQASM.Types.Neg) = "-" * _getstr(tok.val)
_getstr(call::OpenQASM.Types.Call) = string(call.name) * "(" * _getstr(call.args) * ")"

# Parse classical gate parameters; one Float64 per OpenQASM argument.
_parse_angle_token(cargs) = Float64.(eval.(Meta.parse.(_getstr.(cargs))))
...
function _translate_gate(::Val{:rx}, qubits, cargs)
    theta = only(_parse_angle_token(cargs))
...
function _translate_gate(::Val{:p}, qubits, cargs)
    ...
    lambda = only(_parse_angle_token(cargs))
...
function _translate_gate(::Val{:u3}, qubits, cargs)
    ...
    theta, phi, lambda = _parse_angle_token(cargs)
...
function _translate_gate(::Val{:u2}, qubits, cargs)
    ...
    theta = pi/2
    phi, lambda = _parse_angle_token(cargs)
...
function _translate_gate(::Val{:cu}, qubits, cargs)
    ...
    theta, phi, lambda, gamma = _parse_angle_token(cargs)
...
function readqasm(filepath::String)
    ...
    # Lay the qubit registers out back to back; `offsets` maps a register name to its first qubit.
    offsets = Dict{String,Int}()
    nq = 0
    for decl in statements
        decl isa OpenQASM.Types.RegDecl && decl.type.str == "qreg" || continue
        offsets[decl.name.str] = nq
        nq += parse(Int, decl.size.str)
    end
    if isempty(offsets)
    ...
    for instruction in statements
        ...
        elseif instruction isa OpenQASM.Types.RegDecl || instruction isa OpenQASM.Types.Include ||
               instruction isa OpenQASM.Types.Barrier
            continue
        elseif !(instruction isa OpenQASM.Types.Instruction)
            error("Unsupported QASM statement: $(instruction).")
        end

        gate_name_str = instruction.name
        qubits = [_resolve_qubit(q, offsets, gate_name_str) for q in instruction.qargs]
...
# Map a QASM bit reference onto a one-based global qubit index.
function _resolve_qubit(bit, offsets, gate_name)
    regname = bit.name.str
    if !haskey(offsets, regname)
        error("Gate '$gate_name' acts on unknown qubit register '$regname'.")
    end
    if isnothing(bit.address)
        error("Gate '$gate_name' is applied to the whole register '$regname'. Index the qubits explicitly.")
    end
    return offsets[regname] + parse(Int, bit.address.str) + 1
end
